-- #####################################################################
-- #  SCRIPT 09: PAQUETES PL/SQL (ESPECIFICACION + CUERPO)            #
-- #  Los 5 paquetes que implementan los 25 requerimientos.          #
-- #####################################################################
--
-- ORDEN DE EJECUCION (respetar por dependencias):
--   PARTE A -> CONN_SEGURIDAD  : PKG_SEGURIDAD (SEG01-05)
--              (los demas paquetes lo llaman para auditar/validar)
--   PARTE B -> CONN_NEGOCIO    : PKG_VALOR, PKG_ADMINISTRACION, PKG_CONSULTAS
--   PARTE C -> CONN_REPORTES   : PKG_REPORTES
--
-- IMPORTANTE: antes de este script deben existir los GRANT del
-- script 11 (EXECUTE sobre pkg_seguridad hacia negocio/reportes) para
-- que PKG_VALOR y PKG_ADMINISTRACION compilen. Si se ejecuta el
-- proyecto en orden 01..12, ejecutar el bloque de GRANTS de EXECUTE
-- del script 11-PRE (ver nota) o mover esos grants aqui.
--
-- Para simplificar, este script asume que ya se otorgo:
--   GRANT EXECUTE ON c##g01_seguridad.pkg_seguridad TO c##g01_negocio;
--   GRANT EXECUTE ON c##g01_seguridad.pkg_seguridad TO c##g01_reportes;
-- (incluidos tambien en el script 11).
-- =====================================================================


-- #####################################################################
-- PARTE A : PKG_SEGURIDAD  (ejecutar en CONN_SEGURIDAD)
-- #####################################################################
